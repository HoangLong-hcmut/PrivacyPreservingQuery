...
MOCK_DIAGNOSES = [
    {
        ...
        'staff_id': (i % 5) + 1,
        'disease_name': 'Flu',
        'visit_date': date(2023, 1, 1) # Renamed to visit_date
    }
...
class MockDatabase:
    ...
    def execute_query(self, sql, params=None):
        # Basic SQL simulator for mocking purposes
        sql = sql.lower()
        if "count(distinct patient_id)" in sql:
            ...
            # Placeholder for SUM queries if needed for future tests
            return [{'SUM(dummy)': 100}]
        return []
    ...
    def _filter_data(self, sql):
        ...
        # Simulated filtering logic matching expected test queries
        if "where" in sql:
            if "dob<" in sql_norm:
                return data[:len(data)//2]
            if "dob=" in sql_norm:
                # Simulate singling out
